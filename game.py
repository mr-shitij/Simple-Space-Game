...
def game_Over():
	...
	screen.blit(pygame.transform.scale(pygame.image.load('./asset/gameOver.png'), (int(border_width), int(border_height /2))), (border_startX, border_startY / 2))
	pygame.display.update()
	sleep(5)
...
def display_wilans():
	...
	for pos in range(len(wilans_positions[0])):
		if (wilans_positions[1][pos] >= hero_positionY and wilans_positions[1][pos] <= hero_positionY + hero.get_rect().size[1]) or (wilans_positions[1][pos]  + wilan.get_rect().size[1] >= hero_positionY and wilans_positions[1][pos]  + wilan.get_rect().size[1] <= hero_positionY + hero.get_rect().size[1]) and game_over == False:
			if (wilans_positions[0][pos] >= (hero_positionX + 10) and wilans_positions[0][pos] <= (hero_positionX + hero.get_rect().size[0]) - 10) or (wilans_positions[0][pos]  + wilan.get_rect().size[0] >= (hero_positionX + 10) and wilans_positions[0][pos]  + wilan.get_rect().size[0] <= (hero_positionX + hero.get_rect().size[0]) - 10):
				global hero_life
				if hero_life == 0:
					game_over = True
					break
...
def get_data():
	...
	return int(requests.get("http://192.168.1.103").text)
