...
pygame.init()
# pygame.mixer.init(frequency=45100) 421`
...
def process_bullets():
	...
	for bullet in bullets:
		for position in range(len(wilans_positions[0])):
			try:
				if wilans_positions[0][position] <= bullet[0] <= wilans_positions[0][position] + wilan.get_rect().size[0]:
					if wilans_positions[1][position] <= bullet[1] <= wilans_positions[1][position] + wilan.get_rect().size[1]:
						play_blast_audio()
